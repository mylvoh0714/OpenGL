...
def log(R):
    cos_th = (R[0][0]+R[1][1]+R[2][2]-1)/2
    th = np.arccos(cos_th)

    ret = np.array([0.,0.,0.])
    ret[0] = (R[2][1]-R[1][2])/(2*np.sin(th))
    ret[1] = (R[0][2]-R[2][0])/(2*np.sin(th))
    ret[2] = (R[1][0]-R[0][1])/(2*np.sin(th))
...
def interpolateRotVec(rv1,rv2,t):
    ret = np.array([0.,0.,0.])
    for i in range(3):
        ret[i] = lerp(rv1[i],rv2[i],t)
    return exp(ret)

def interpolateZYXEuler(euler1, euler2, t):
    ret = np.array([0.,0.,0.])
    for i in range(3):
        ret[i] = lerp(euler1[i],euler2[i],t)
...
gVisibles = [True,False,False,False] # visible flags for slerp, interpolateZYXEuler
                                     # interpolateRotVec, interpolateRotMat
def render(ang):
